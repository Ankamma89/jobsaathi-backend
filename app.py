...
from db import (
    ...
    chat_details_collection,
    connection_details_collection,
)
from helpers import (
    query_update_billbot,
    add_html_to_db,
    analyze_resume,
...
def get_user(token):
    ...
    if user_id is None:
        ...
        current_user = user_details_collection.find_one(
...
def about_us():
    ...
    resp = make_response(
        render_template(
            "about_us.html", user_name=user_name, user_logged_in=user_logged_in
        )
    )
    return resp
...
def contact_us():
    ...
    resp = make_response(
        render_template(
            "contact_us.html", user_name=user_name, user_logged_in=user_logged_in
        )
    )
    return resp
...
def search_jobs(user):
    ...
    pipeline = [
        ...
        {"$project": {"_id": 0, "job_details._id": 0}},
    ]
...
def alljobs(user):
    ...
    onboarding_details = onboarding_details_collection.find_one(
        {"user_id": user_id}, {"_id": 0}
    )
...
def dashboard(user):
    ...
    if purpose == "hirer":
        ...
        if approved_by_admin:
            ...
            page_size = 7  # Number of documents per page
            total_elements = len(
                list(jobs_details_collection.find({"user_id": user_id}, {"_id": 0}))
            )
            total_pages = calculate_total_pages(total_elements, page_size)
            skip = (page_number - 1) * page_size
            pipeline = [
            ...
            all_published_jobs = list(
                jobs_details_collection.find(
                    {"user_id": user_id, "status": "published"}, {"_id": 0}
                )
            )
            total_selected_candidates = list(
                candidate_job_application_collection.find(
                    {"hirer_id": user_id, "status": "Accepted"}, {"_id": 0}
                )
            )
            stats = {
        ...
        if not resume_built:
            return redirect("/billbot")
        resume_skills_string = resume_details_collection.find_one(
            {"user_id": user_id}, {"skills": 1}
        ).get("skills")
        resume_skills = [
            skill.strip().lower() for skill in resume_skills_string.split(",")
        ]
        regex_patterns = []
        ...
        for idx, job in enumerate(all_jobs):
            if applied := candidate_job_application_collection.find_one(
                {"job_id": job.get("job_id"), "user_id": user_id}, {"_id": 0}
            ):
                pass
            else:
                all_updated_jobs.append(job)
        profile_details = profile_details_collection.find_one(
            {"user_id": user_id}, {"_id": 0}
        )
        return jsonify(
            {
...
def applied_jobs(user):
    ...
    if pageno is not None:
        page_number = int(pageno)
    page_size = 7  # Number of documents per page
    length_pipeline = [{"$match": {"user_id": user_id}}, {"$project": {"_id": 0}}]
    total_elements = len(
        list(candidate_job_application_collection.aggregate(length_pipeline))
    )
    total_pages = calculate_total_pages(total_elements, page_size)
    skip = (page_number - 1) * page_size
    pipeline = [
        ...
        {"$project": {"_id": 0, "job_details._id": 0, "user_details._id": 0}},
        {"$skip": skip},  # Skip documents based on the calculated skip value
        {"$limit": page_size},  # Limit the number of documents per page
    ]
...
def profile_update(user):
    ...
    if request.method == "POST":
        ...
        if (
            "profile_pic" in request.files
            and str(request.files["profile_pic"].filename) != ""
        ):
            profile_pic = request.files["profile_pic"]
            profile_pic_link = upload_file_firebase(
                profile_pic, f"{user_id}/profile_pic.png"
            )
    ...
    if profile_details := profile_details_collection.find_one(
        {"user_id": user_id}, {"_id": 0}
    ):
        if purpose == "jobseeker":
        ...
        abort(
            500,
            {"message": f"DB Error: Profile Details for user_id {user_id} not found."},
        )


@app.route(
    "/public/candidate/<string:user_id>",
    methods=["GET", "POST"],
    endpoint="public_candidate_profile",
)
def public_candidate_profile(user_id):
    pipeline = [
        ...
        {"$project": {"_id": 0, "resume_details._id": 0}},
    ]
...
@app.route(
    "/upload_intro_candidate", methods=["POST"], endpoint="upload_intro_candidate"
)
@newlogin_is_required
@is_candidate
def upload_intro_candidate(user):
    user_id = user.get("user_id")
    if (
        "intro_video" in request.files
        and str(request.files["intro_video"].filename) != ""
    ):
        intro_video = request.files["intro_video"]
        intro_video_link = upload_file_firebase(
            intro_video, f"{user_id}/intro_video.mp4"
        )
        profile_details_collection.update_one(
            {"user_id": user_id}, {"$set": {"intro_video_link": intro_video_link}}
        )
        return redirect("/profile")
...
def login_hirer():
    ...
    if request.method == "POST":
        user = user_details_collection.find_one(
            {"email": form_data.get("email")}, {"_id": 0}
        )
        if user:
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            if password == user.password:
                token = jwt.encode(
                    {"public_id": user.user_id, "exp": "30000000000000000000000000"},
                    APP_SECRET,
                )
                flash("Successfully Logged In")
                return (
                    jsonify(
                        {
                            "message": "logged in",
                            "data": {"token": "token", "onboarded": False},
                        }
                    ),
                    200,
                )
...
@app.route(
    "/register-jobseeker", methods=["GET", "POST"], endpoint="register_jobseeker"
)
def register_jobseeker():
    ...
    if request.method == "POST":
        if (
            user_details_collection.find_one(
                {"email": form_data.get("email")}, {"_id": 0}
            )
            is None
        ):
...
def chatbot(user):
    user_id = user.get("user_id")
    if onboarding_details := onboarding_details_collection.find_one(
        {"user_id": user_id}, {"_id": 0}
    ):
        ...
        if phase == "1":
            ...
            if resume_uploaded:
                messages = [
                    {"user": "billbot", "msg": "Hi, I am BillBot."},
                    {
                        "user": "billbot",
                        "msg": f"I see you have already uploaded a <a href={resume_link} target=_blank>Resume</a>. Click Yes, if you want to upload another resume and hit no to use BillBot to develope a resume using AI!",
                    },
                ]
            else:
                messages = [
                    {"user": "billbot", "msg": "Hi, I am BillBot."},
                    {"user": "billbot", "msg": "Do you have a pre-built resume?"},
                ]
            ...
            if resume_details := resume_details_collection.find_one(
                {"user_id": user_id}, {"_id": 0}
            ):
                ...
                return jsonify(
                    {
                        ...
                        "nxt_build_status": nxt_build_status,
                    }
                )
            else:
...
def edit_mdresume(user):
    ...
    if request.method == "POST":
        ...
        resume_details_collection.update_one(
            {"user_id": user_id}, {"$set": {"resume_html": resume_html}}
        )
    ...
    if resume_details := resume_details_collection.find_one(
        ...
        abort(
            500,
            {
                "messages": f"Resume Deatails for user_id {user_id} unavailable! Contact Admin!"
            },
        )
...
def resume_build(user):
    ...
    return {
        ...
        "nxt_messages": outbound_messages(nxt_build_status),
        "nxt_build_status": nxt_build_status_,
    }
...
def resume_built(user):
    ...
    resume_html = form_data.get("resume_html")
    user_id = user.get("user_id")
    onboarding_details_collection.update_one(
        {"user_id": user_id}, {"$set": {"resume_built": True}}
    )
    resume_details_collection.update_one(
        {"user_id": user_id}, {"$set": {"resume_html": resume_html}}
    )
...
def resume_upload(user):
    ...
    if "resume" in request.files:
        ...
        if resume_details := resume_details_collection.find_one(
            ...
            resume_details_collection.insert_one(
                {"user_id": user_id, "resume_link": resume_link}
            )
        ...
        return {"resume_link": resume_link}
...
def callback():
    ...
    pipeline = [
        ...
        {"$project": {"_id": 0, "onboarding_details._id": 0}},
    ]
...
def onboarding(user):
    ...
    if request.method == "POST":
        if user_id is None:
            ...
            if user_details := user_details_collection.find_one(
                {"user_id": user_id}, {"_id": 0}
            ):
                ...
                if user_details.get("onboarded") == False:
                    ...
                    if purpose and purpose == "jobseeker":
                        ...
                        profile_data = {
                            ...
                            "email": user_details.get("email"),
                            "company_representative_mobno": onboarding_details.get(
                                "company_representative_mobno"
                            ),
                        }
                        if (
                            "company_logo" in request.files
                            and str(request.files["company_logo"].filename) != ""
                        ):
                            company_logo = request.files["company_logo"]
                            company_logo_link = upload_file_firebase(
                                company_logo, f"{user_id}/company_logo.png"
                            )
                        ...
                        abort(
                            500,
                            {
                                "message": "Onboarding couldn't be completed due to some technical issue!"
                            },
                        )
...
@app.route(
    "/onboarding-recruiter", methods=["GET", "POST"], endpoint="onboardingRecruiter"
)
@newlogin_is_required
...
@app.route(
    "/onboarding-jobseeker", methods=["GET", "POST"], endpoint="onboardingJobSeeker"
)
@newlogin_is_required
...
def edit_job(user, job_id):
    ...
    if request.method == "POST":
        ...
        jobs_details_collection.update_one(
            {"user_id": str(user_id), "job_id": str(job_id)}, {"$set": incoming_details}
        )
        return redirect("/dashboard")
    if job_details := jobs_details_collection.find_one(
        {"user_id": str(user_id), "job_id": str(job_id)}, {"_id": 0}
    ):
...
def delete_job(user, job_id):
    user_id = user.get("user_id")
    if request.method == "POST":
        jobs_details_collection.delete_one(
...
def save_job(user, job_id):
    user_id = user.get("user_id")
    if _ := saved_jobs_collection.find_one(
...
def edit_task(user, task_id):
    ...
    if request.method == "POST":
        incoming_details = dict(request.form)
        tasks_details_collection.update_one(
            {"user_id": str(user_id), "task_id": str(task_id)},
            {"$set": incoming_details},
        )
        return jsonify({"message": "task editedd successfully"}), 200
    if task_details := tasks_details_collection.find_one(
        {"user_id": str(user_id), "task_id": str(task_id)}, {"_id": 0}
    ):
        return jsonify({"task_details": task_details})


@app.route(
    "/remove_saved_job/<string:job_id>", methods=["POST"], endpoint="remove_saved_job"
)
@newlogin_is_required
@is_candidate
...
def apply_job(user, job_id):
    ...
    if request.method == "POST":
        if job_details := jobs_details_collection.find_one(
            ...
            flash(
                "Successfully Applied for the Job. Recruiters will get back to you soon, if you are a good fit."
            )
...
def change_job_status(candidate_user_id):
    ...
    candidate_job_application_collection.update_one(
        {"job_id": job_id, "user_id": candidate_user_id}, {"$set": {"status": status}}
    )
    return redirect(f"/responses/job/{job_id}")


@app.route(
    "/responses/job/<string:job_id>", methods=["GET", "POST"], endpoint="job_responses"
)
@newlogin_is_required
@is_hirer
@is_onboarded
def job_responses(user, job_id):
    if job_details := jobs_details_collection.find_one(
        {"job_id": job_id}, {"_id": 0, "job_title": 1, "mode_of_work": 1}
    ):
        pageno = request.args.get("pageno")
        page_number = 1  # The page number you want to retrieve
        if pageno is not None:
        ...
        page_size = 7  # Number of documents per page
        total_elements = len(
            list(candidate_job_application_collection.find({"job_id": job_id}))
        )
        total_pages = calculate_total_pages(total_elements, page_size)
        skip = (page_number - 1) * page_size
        pipeline = [
...
def alltasks(user):
    ...
    pipeline = [
        ...
        {"$project": {"_id": 0, "task_details._id": 0}},
        {"$skip": skip},  # Skip documents based on the calculated skip value
        {"$limit": page_size},  # Limit the number of documents per page
    ]
    all_tasks = list(tasks_details_collection.aggregate(pipeline))
...
@app.route(
    "/apply/task/<string:task_id>", methods=["GET", "POST"], endpoint="apply_task"
)
@newlogin_is_required
@is_candidate
def apply_task(user, task_id):
    ...
    if job_details := list(tasks_details_collection.aggregate(pipeline)):
        ...
        if task_details.get("status") == "published":
            if candidate_task_proposal_collection.find_one(
                {"user_id": user_id, "task_id": task_id}, {"_id": 0}
            ):
                applied = True
            else:
                applied = False
...
@app.route(
    "/responses/task/<string:task_id>",
    methods=["GET", "POST"],
    endpoint="task_responses",
)
@newlogin_is_required
@is_hirer
@is_onboarded
def task_responses(task_id):
    if task_details := tasks_details_collection.find_one(
        {"task_id": task_id}, {"_id": 0, "task_title": 1, "mode_of_work": 1}
    ):
        pageno = request.args.get("pageno")
        page_number = 1  # The page number you want to retrieve
        if pageno is not None:
        ...
        page_size = 7  # Number of documents per page
        total_elements = len(
            list(candidate_task_proposal_collection.find({"task_id": task_id}))
        )
        total_pages = calculate_total_pages(total_elements, page_size)
        skip = (page_number - 1) * page_size
        pipeline = [
...
@app.route(
    "/chat/<string:incoming_user_id>/<string:job_id>",
    methods=["GET", "POST"],
    endpoint="specific_chat",
)
@newlogin_is_required
def specific_chat(user, incoming_user_id, job_id):
    ...
    if request.method == "POST":
        ...
        channel_id = (
            f"{user_id}_{incoming_user_id}_{job_id}"
            if purpose == "jobseeker"
            else f"{incoming_user_id}_{user_id}_{job_id}"
        )
    ...
    if onboarding_details := onboarding_details_collection.find_one(
        {"user_id": incoming_user_id}, {"_id": 0}
    ):
        name = (
            onboarding_details.get("company_name")
            if purpose == "jobseeker"
            else onboarding_details.get("jobseeker_name")
        )
        pipeline = [
            {
                "$match": {
        ...
        job_details = jobs_details_collection.find_one(
            {"job_id": job_id}, {"_id": 0, "job_title": 1}
        )
        meet_details = {"meetLink": f"http://127.0.0.1:5000/meet/{channel_id}"}
        return jsonify(
            {
...
def initiate_chat(user):
    ...
    job_id = form_data.get("job_id")
    if connection_details := connection_details_collection.find_one(
        {"jobseeker_id": jobseeker_id, "hirer_id": user_id}, {"_id": 0}
    ):
        pass
    else:
        if _ := candidate_job_application_collection.find_one(
            {"user_id": jobseeker_id, "hirer_id": user_id, "job_id": job_id}, {"_id": 0}
        ):
            connection_details = {
            ...
            candidate_job_application_collection.update_one(
                {"user_id": jobseeker_id, "hirer_id": user_id, "job_id": job_id},
                {"$set": {"chat_initiated": True}},
            )
        else:
...
def meeting(user, channel_id):
    ...
    if onboarding_details := list(
        onboarding_details_collection.aggregate(hirer_pipeline)
    ):
    ...
    if onboarding_details := list(
        onboarding_details_collection.aggregate(candidate_pipeline)
    ):
