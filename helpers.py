...
1. **Only** return the updated JSON in valid JSON format using double quotes.
...
Analyze the instruction and update only the relevant sections in the JSON template based on the profile and statement. Return **only** the updated JSON template in a valid and clean JSON file format. Do not include any extra text or explanation.
"""
...
def query_update_billbot(user_id, statement, build_status):
    resume_html,json_template,resume_json = get_resume_html_db(user_id)
...
def get_resume_html_db(user_id):
    if resume_data := resume_details_collection.find_one({"user_id": user_id}):
        ...
        resume_json = resume_data.get("resume_json")
        json_template=resume_data.get("json_template")
        return str(resume_html),str(json_template),str(resume_json)
    else:
        return ""
